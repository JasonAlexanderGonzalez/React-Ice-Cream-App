...
// Importa el sonido de listo
import listoSound from "../sounds/listo.mp3";
...
const Orden = ({ orden }) => {
  ...
  if (!helado || orden.estado !== 3) {
    return null;
  }

  // Función para reproducir el sonido de listo
  const reproducirSonido = () => {
    const audio = new Audio(listoSound);
    audio.play();
  };
    ...
    <div className="bg-white p-4 rounded-lg shadow-md shadow-teal-400 flex flex-col border-teal-400 border">
      ...
      <div className="flex-grow bg-white p-4 rounded-lg shadow-md mb-3">
        ...
        <div className="block mt-5">
          <span className="block text-center text-gray-800 mb-2 font-bold bg-orange-200 border-l-4 border-orange-600"> {estadoLec}</span>
          <div className="flex">
            <button
          ...
          {aceptado && <p className="text-green-500 mt-2">Orden Entregada</p>}
        </div>
      </div>
    </div>
  );
